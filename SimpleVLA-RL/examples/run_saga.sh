...
# Submit from SimpleVLA-RL root:  sbatch examples/run_saga.sh
...
# SAGA-specific
APD_PLANS_PATH="${APD_PLANS_PATH:-$REPO_ROOT/../APD_plans_scaled.json}"
...
if [[ ! -f "$APD_PLANS_PATH" ]]; then
    echo "Error: APD_PLANS_PATH not found: $APD_PLANS_PATH"
    echo "Set APD_PLANS_PATH to your APD_plans_scaled.json"
    exit 1
fi
...
echo "SAGA RL (Apptainer) — SFT: $SFT_MODEL_PATH  Experiment: $EXPERIMENT_NAME"
echo "  R_task coef=$VERIFIER_REWARD_COEF  KL=$KL_COEF  adv=$ADV_ESTIMATOR"
