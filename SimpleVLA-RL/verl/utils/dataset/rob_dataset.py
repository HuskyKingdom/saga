...
class LIBERO_Dataset(Dataset):
    ...
    # Mapping from combined suite alias to constituent suites
    COMBINED_SUITES = {
        "libero_4_task_suites": ["libero_spatial", "libero_object", "libero_goal", "libero_10"],
    }
    ...
    def _read_files_and_tokenize(self):
        ...
        # --- Combined task suite (e.g. libero_4_task_suites) ---
        if self.task_suite_name in self.COMBINED_SUITES:
            sub_suites = self.COMBINED_SUITES[self.task_suite_name]
            for suite_name in sub_suites:
                ...
                for task_id in range(num_tasks_in_suite):
                    ...
                    for i in trials_range:
                        data = {
                            # NOTE: store real suite name so env_worker can look it up directly
            ...
            print(f'[libero_4_task_suites] combined dataset len: {len(self.dataframe)}')

        # --- Single task suite ---
        elif self.task_suite_name in ["libero_10", "libero_90", "libero_goal",  "libero_object",  "libero_spatial"]:
