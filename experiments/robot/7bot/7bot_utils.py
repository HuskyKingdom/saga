"""Utils for evaluating policies in real-world s_bot environments."""
...
from experiments.robot.s_bot.real_env import make_real_env
...
def get_s_bot_env():
    """Initializes and returns the s_bot environment."""
...
def resize_image_for_preprocessing(img):
    ...
    in the s_bot data preprocessing script, which is used before converting the dataset to RLDS.
    """
    s_bot_PREPROCESS_SIZE = 256
    img = np.array(
        Image.fromarray(img).resize((s_bot_PREPROCESS_SIZE, s_bot_PREPROCESS_SIZE), resample=Image.BICUBIC)
    )  # BICUBIC is default; specify explicitly to make it clear
    return img


def get_s_bot_image(obs):
...
def get_s_bot_wrist_images(obs):
